...
class Eval:
    ...
    def run(self, x):
        val = self.session.run(self.gragh.get_tensor_by_name("y:0"), feed_dict = {
            self.gragh.get_tensor_by_name("x:0"): np.reshape(x, [1] + INPUT_SIZE),
        })
        return val.tolist()[0]
...
if __name__ == "__main__":
    ...
    validations_x = np.reshape(mnist.validation.images[BATCH_SIZE], [-1] + INPUT_SIZE)
    validations_y = mnist.validation.labels[:BATCH_SIZE]
    cnn_lenet.batch_train(conf,
        next_batch, mnist,
        lambda: (validations_x, validations_y))
